			            ...
			            <div class="card-body pt-2">
							...
							<div class="inbox-leftbar text font-15 fw-bold" style="margin-top: 0px; padding: 0px; min-height: 670px">
									<a href="javascript:void(0);" onclick="getRepoData()" style="display: block; margin-bottom: 10px; text-align: center;">
										<button id="myDataRoomBtn"type="button" class="btn btn-primary waves-effect waves-light fw-bold active" style="font-size: 20px; padding: 10px 70px 10px; border: none">내 보관함</button>
									</a>
...
<!--                         <a class="custom-accordion-title text-reset d-block collapsed" data-bs-toggle="collapse" href="#collapseSeven" aria-expanded="false" aria-controls="collapseSeven"> -->
<!-- 							카테고리명 -->
<!--                         </a> -->
<!--                     </h5> -->
<!--                 </div> -->
...
//                      console.log("cnt : ", cnt);
                     ...
                     let twName = filePath.split("\\")[2];
//                      console.log("twName : ", twName);
                     let sliced = item.sliced;
				     
                     // 사진 아닐 때
                     // 파일 확장자별로 기본 아이콘 다르게 띄우기
//                      let noFileIcon = "\\resources\\src\\images\\defaultFileImage\\noFileIconBlack.png";
//                      let defaultIcon = "\\resources\\src\\images\\defaultFileImage\\defaultIcon.png";
//                      let pdfIcon = "\\resources\\src\\images\\defaultFileImage\\PDFIcon.png";
//                      let txtIcon = "\\resources\\src\\images\\defaultFileImage\\TXTIcon.png";
                     
                     let isImage = item.image;
                     console.log("item.isImage : ", isImage);
                     let container = null;
                     let gallary = "";
                     let childElement = null;
                     
//                      if(isImage == "false") filePath = defaultIcon;					// default
//                      if(isImage == "false" && sliced == "pdf") filePath = pdfIcon;	// pdf
//                      if(isImage == "false" && sliced == "pdf") filePath = pdfIcon;	// pdf
//                      if(isImage == "false" && sliced == "pdf") filePath = pdfIcon;	// pdf
//                      if(isImage == "false" && sliced == "pdf") filePath = pdfIcon;	// pdf
                     
                     
                     console.log("그래서filePath는????????????? : ", filePath);
                     var test = filePath.split("\\");
                     
                     let txt = "";
                     txt += `<div class="card m-2" style="flex: 1 1 20%; border: 1px solid lightgray">`;
                     txt += `<a class="fs-3 fw-bold " id="word1" style="color:black; text-align:center; font-size:13px; padding:0px;" href="\\resources\\upload\\` + filePath + `" download = "` + item.fileRealName + `">`;
                     txt += `<img style="width: 220px; height: 200px" class="card-img-top embed-responsive-item" src="\\resources\\upload\\` + filePath + `" alt="no Image"/>`;
                     txt += `<p style=" text-align: center; margin-bottom: 15px">`;
	                 txt += `\${item.fileName}`;
	                 txt +=  `&nbsp;&nbsp;`;
	                 txt +=    	`
	                	 <a href="javascript:void(0)" onclick="putMyRepo('\${memberId}', '`+test+`', '\${item.fileRealName}')" 
	                	 class="text-center mt-1 fs-3 btn btn-primary" style="width:200px;text-align:center;"> 보관함에 담기 </a>
	                 			`;
                     txt +=  `</p>`;
                     txt += `</a>`;
                     txt += `</div>`;
                     ...
                     cardTitle.id = "twName";
                     cardTitle.innerText =  "메인작업명 : " + twName;
                     cardHeader.append(cardTitle);
                     
                     let p = document.createElement("p");
                     p.className = "text-muted mb-0 fw-bold " + i;
                     p.id = "cateName";
                     p.innerText = "카테고리명 : " + cateName;
                     cardHeader.append(p);
...
//                      console.log("cardBody : ", cardBody.style);
                     ...
                     if(i > 0) {
	                        ...
	                        let curTwName = data[i-1].filePath.split("\\")[2];
...
function getRepoData() {
   ...
   $.ajax({
      ...
      success : function(data) {
		...
		aDown.setAttribute("id","downloadTab");
		aDown.setAttribute("href","#download");
		...
		aUp.setAttribute("id","uploadTab");
		aUp.setAttribute("href","#upload");
        ...
        if(data.length == 0) {
			...
			paneDown.className = "tab-pane active";
			paneDown.id = "download";
			...
			paneUp.id = "upload";
			paneUp.style.cssText = "display: none; flex-wrap: wrap;";
...
// 				alert("올렷다");
	         ...
	         $.each(data, function(i, item) {
// 	            let downloadContent = document.querySelector("#download");   // 담은 파일 영역
// 	            let uploadContent = document.querySelector("#upload");       // 올린 파일 영역
	            ...
	            let txt = ``;
		            ...
		            txt += `<p class="fs-3 fw-bold "style="color:black; text-align: center;">` + item.fileRealName + `</p>`;
		            txt += `</a>`;
		            txt += `</div>`;	
