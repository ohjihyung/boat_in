...
function noticeInsertForm(data) {
	...
	text += `							<select id="noticeCategory" style="width: 230px; height: 45px;">`;
	...
	text += `						<input type="text" id="sysNTitle" name="sysNTitle" value="" class="form-control" placeholder="제목을 입력하세요" style="font-size: 1.2em">`;
	text += `					</div>`;
	text += `				</div>`;
	text += `				<div class="form-group">`;
	...
	text += `						<input type="button" value="목록" id="listBtn" class="btn btn-success waves-effect waves-light" onclick="noticeList()" style="font-size: 1.2em">`; 
							if (data) {
	  text += `						<input type="submit" value="수정" id="updateBtn" class="btn btn-primary waves-effect waves-light" onclick="noticeUpdate(\${data.sysNNo})" style="font-size: 1.2em">`;
							} else {
	  text += `						<input type="submit" value="등록" id="formBtn" class="btn btn-primary waves-effect waves-light" onclick="noticeInsert()" style="font-size: 1.2em">`;
							}
	...
	document.querySelector("#noticeQuill-container").innerHTML = `<div id="noticeQuill" class="form-control" style="height:550px;"></div>`;
...
//         [{ 'font': [] }],
    ...
    $(".ql-toolbar").css("font-size", "1.3em");
}
